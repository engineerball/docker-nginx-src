...
NGINX_LDAP_DOWNLOAD_URL="https://github.com/kvspb/nginx-auth-ldap/archive/v${LDAP_VERSION}.tar.gz"

apt-get update
DEBIAN_FRONTEND=noninteractive apt-get install -y gcc g++ make libc6-dev libpcre++-dev libssl-dev libxslt-dev libgd2-xpm-dev libgeoip-dev libldap2-dev
...
download_and_extract "${NGINX_LDAP_DOWNLOAD_URL}" "${NGINX_SETUP_DIR}/nginx-ldap-module"
...
./configure --prefix=/usr/share/nginx --conf-path=/etc/nginx/nginx.conf --sbin-path=/usr/sbin \
  ...
  --add-module=${NGINX_SETUP_DIR}/nginx-ldap-module \
